...
#if defined(_WIN32)
    // SHOpenFolderAndSelectItems — the only Explorer API that can select more
    // than one file. Needs COM; SDL may have initialized it with a different
    // threading model, so only pair CoUninitialize with our own init.
...
void App::Render() {
    ...
    if (popupVisibleThisFrame) {
        ...
        if (!progress.running && !progress.finished && !progress.error) {
            ...
            // The paths the exporter actually wrote, fallback naming included.
